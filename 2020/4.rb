raw_passports = DATA.read.split("\n\n")

passports = raw_passports.map do |raw|
  raw.split.map{ |f| f.split(":") }.to_h
end

fields = {
  "byr" => -> (v) { v.length == 4 && v.to_i >= 1920 && v.to_i <= 2002 },
  "iyr" => -> (v) { v.length == 4 && v.to_i >= 2010 && v.to_i <= 2020 },
  "eyr" => -> (v) { v.length == 4 && v.to_i >= 2020 && v.to_i <= 2030 },
  "hgt" => -> (v) { (v.end_with?("cm") && v.to_i >= 150 && v.to_i <= 193) || (v.end_with?("in") && v.to_i >= 59 && v.to_i <= 76) },
  "hcl" => -> (v) { v.match? /\A\#[0-9a-f]{6}\z/ },
  "ecl" => -> (v) { ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'].include?(v) },
  "pid" => -> (v) { v.match? /\A[0-9]{9}\z/}
}

puts "With present fields: #{passports.filter{|p| (fields.keys - p.keys).length == 0}.count}"

valid = passports.filter do |p|
  fields.all? do |key, value|
    !p[key].nil? && fields[key].call(p[key])
  end
end

puts "With present and correct fields: #{valid.count}"
